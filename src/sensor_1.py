...
class Sensor_1(btle.DefaultDelegate):
...
def sensor_1():
    try:
        ...
        pulse1.setDelegate( Sensor_1('pulse1') )


        # Setup to turn pulse1 notifications on
        svc = pulse1.getServiceByUUID("49535343-fe7d-4ae5-8fa9-9fafd205e455")
        ch = svc.getCharacteristics("49535343-1e4d-4bd9-ba61-23c647249616")[0]
        pulse1.writeCharacteristic(ch.valHandle+1, SETUP_DATA, withResponse=True)
        ...
        while True:
            ...
            if pulse1.waitForNotifications(2.0):
                # handleNotification() was called for pulse1. Iterates counter and outputs when counter limit met
                continue
            else:
                raise Exception
...
#         traceback.print_exc()--------------------------uncomment to print traceback
        print("\nSensor 1 waiting for connection...")
